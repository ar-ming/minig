...
public class Threader {
	 ...
	 * Threadable objects, and the Threader constructs a connected 'graph' of
	 * messages
	 * 
	 * @param messages
	 * @return null if messages == null or root.child == null
	 */
	public IThreadable thread(List<? extends IThreadable> messages) {
		if (messages == null)
			return null;
		...
		IThreadable result = (root.child == null ? null : root.child.threadable);
		root.flush();
		root = null;
	...
	private void buildContainer(IThreadable threadable) {
		String id = threadable.getMessageID();
		...
		{
			String[] references = threadable.getReferences();
	...
	private void pruneEmptyContainers(ThreadContainer parent) {
		...
		for (prev = null, container = parent.child, next = container.next; container != null; prev = container, container = next, next = (container == null ? null
				: container.next)) {
	...
	private void gatherSubjects() {
		...
		HashMap<String, ThreadContainer> subjectTable = new HashMap<String, ThreadContainer>(
				(int) (count * 1.2), (float) 0.9);
		count = 0;
		...
		for (ThreadContainer c = root.child; c != null; c = c.next) {
			...
			String subj = threadable.getSimplifiedSubject();
		...
		for (prev = null, c = root.child, rest = c.next; c != null; prev = c, c = rest, rest = (rest == null ? null
			...
			if (old.threadable == null && c.threadable == null) {
				...
				c.child = null;
					...
					|| (c.threadable != null && c.threadable.subjectIsReply() && !old.threadable
							.subjectIsReply())) {
