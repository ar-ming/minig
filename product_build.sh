...
product_name=minig-backend
product_file=`pwd`/plugins/fr.aliasource.webmail.proxy/webmail.product
## END USER VARS
...
find build_directory -name "bin" -type d |xargs rm -fr
find build_directory -name "*.tests" -type d |xargs rm -fr
...
cp ../../scripts/webmail-backend .
cp ../../scripts/webmail-backend-profile .
popd >/dev/null 2>&1
