...
public class ListAvailableFolderMethod extends AbstractClientMethod {

	private String token;

	public ListAvailableFolderMethod(HttpClient hc, String token,
			String backendUrl) {
		super(hc, backendUrl, "/listAvailableFolder.do");
		this.token = token;
	}
		...
		if (doc != null) {
			...
			String[][] values = DOMUtils.getAttributes(
					doc.getDocumentElement(), "folder", new String[] { "name",
							"displayName", "subscribed", "shared" });
			for (int i = 0; i < values.length; i++) {
				ret.add(new Folder(values[i][0], values[i][1], 
						Boolean.valueOf(values[i][2]), Boolean.valueOf(values[i][3])));
			}
		}
		return ret;
	}
